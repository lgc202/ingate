...
import type { Certificate, Gateway, GatewayListener, GatewayRoute, Policy, Service, ServiceEndpoint, ServiceType, TrafficType } from '../data';
import { usePrototype } from '../prototype-context';

export function GatewayPage() {
  const { gateways, routes, certificates, policies, addGateway, updateGateway, deleteGateway } = usePrototype();
  ...
  return (
    <div className="page-stack page-enter">
      ...
      <section className="card table-card">
        ...
        <div className="table-head gateway-columns"><span>网关</span><span>监听入口</span><span>域名</span><span>证书</span><span>路由</span><span>状态</span><span>操作</span></div>
        {visible.length ? visible.map((gateway) => (
          <div key={gateway.id} className="table-row gateway-columns">
            ...
            <span>{gateway.listeners.map(listenerLabel).join(' · ')}</span><span>{gatewayDomains(gateway).join('、')}</span><span>{gatewayCertificateNames(gateway, certificates).join('、') || '—'}</span><strong>{routes.filter((route) => route.gatewayID === gateway.id).length}</strong><StatusBadge state={gateway.state} /><RowActions onDetail={() => setSelected(gateway)} onEdit={() => setEditing(gateway)} onDelete={() => setDeleting(gateway)} />
          </div>
      ...
      {creating ? <CreateGateway gateways={gateways} certificates={certificates} onClose={() => setCreating(false)} onSave={(gateway) => { addGateway(gateway); setCreating(false); setToast('网关已保存，正在发布'); }} /> : null}
      {editing ? <CreateGateway initial={editing} gateways={gateways} certificates={certificates} onClose={() => setEditing(null)} onSave={(gateway) => { updateGateway(gateway); setEditing(null); setToast('网关修改已保存，正在发布'); }} /> : null}
      {deleting ? <DeleteConfirm resourceType="网关" resourceName={deleting.name} blockedReason={routes.some((route) => route.gatewayID === deleting.id) ? `还有 ${routes.filter((route) => route.gatewayID === deleting.id).length} 条路由使用该网关。请先迁移或删除这些路由。` : policies.some((policy) => policy.targets.some((target) => target.kind === '网关' && target.id === deleting.id)) ? '仍有流量策略应用到该网关。请先调整策略的生效范围。' : undefined} onCancel={() => setDeleting(null)} onConfirm={() => { deleteGateway(deleting.id); setDeleting(null); setToast('网关已删除，正在发布'); }} /> : null}
...
function GatewayDetail({ gateway, routes, certificates, policies, onClose }: { gateway: Gateway; routes: GatewayRoute[]; certificates: Certificate[]; policies: Policy[]; onClose: () => void }) {
  ...
  return (
    <Drawer title={gateway.name} description="流量入口与当前生效配置" onClose={onClose} width="wide">
      ...
      <div className="detail-kpis"><Metric label="路由" value={String(routes.length)} note={gateway.state === 'pending' ? '网关配置发布中' : '当前已生效'} /><Metric label="代理实例" value="5 / 5" note="当前环境" /><Metric label="可用率" value="99.99%" note="过去 24 小时" /></div>
...
function CreateGateway({ initial, gateways, certificates, onClose, onSave }: { initial?: Gateway; gateways: Gateway[]; certificates: Certificate[]; onClose: () => void; onSave: (gateway: Gateway) => void }) {
  const [name, setName] = useState(initial?.name ?? '');
  const [listeners, setListeners] = useState<GatewayListener[]>(initial?.listeners.map((listener) => ({ ...listener, bindings: listener.bindings.map((binding) => ({ ...binding })) })) ?? [{ id: crypto.randomUUID(), protocol: 'HTTPS', port: 443, bindings: [{ domain: 'new-api.example.com', certificateID: '' }] }]);
...
export function RoutePage() {
  const { routes, gateways, services, policies, callers, addRoute, updateRoute, deleteRoute } = usePrototype();
  ...
  return (
    <div className="page-stack page-enter">
      ...
      <section className="card table-card">
        ...
        <div className="table-head route-columns"><span>路由</span><span>入口</span><span>匹配规则</span><span>目标服务</span><span>请求</span><span>成功率</span><span>性能</span><span>状态</span><span>操作</span></div>
        {visible.length ? visible.map((route) => <div key={route.id} className="table-row route-columns"><div className="name-cell"><TypeBadge type={route.type} /><div><strong>{route.name}</strong><small>{route.published.join('、')}</small></div></div><div><strong>{route.gatewayName}</strong><small>{route.host}{route.path}</small></div><span>{route.match}</span><span>{route.targets[0]?.serviceName}{route.targets.length > 1 ? ` 等 ${route.targets.length} 个` : ''}</span><strong>{route.requests}</strong><span>{route.successRate}</span><span>{route.latency}</span><StatusBadge state={route.state} /><RowActions onDetail={() => setSelected(route)} onEdit={() => setEditing(route)} onDelete={() => setDeleting(route)} /></div>) : <EmptyState title="没有匹配的路由" description="调整筛选条件，或创建一条新路由。" />}
      </section>
      {selected ? <RouteDetail route={selected} policies={policies} onClose={() => setSelected(null)} /> : null}
      {creating ? <CreateRoute gateways={gateways} services={services} onClose={() => setCreating(false)} onSave={(route) => { addRoute(route); setCreating(false); setToast('路由已保存，正在发布'); }} /> : null}
      {editing ? <CreateRoute initial={editing} gateways={gateways} services={services} onClose={() => setEditing(null)} onSave={(route) => { updateRoute(route); setEditing(null); setToast('路由修改已保存，正在发布'); }} /> : null}
      {deleting ? <DeleteConfirm resourceType="路由" resourceName={deleting.name} blockedReason={callers.some((caller) => caller.permissions.some((permission) => permission.routeID === deleting.id)) ? '仍有调用方被授权访问该路由。请先移除相关权限。' : policies.some((policy) => policy.targets.some((target) => target.kind === '路由' && target.id === deleting.id)) ? '仍有流量策略应用到该路由。请先调整策略的生效范围。' : undefined} onCancel={() => setDeleting(null)} onConfirm={() => { deleteRoute(deleting.id); setDeleting(null); setToast('路由已删除，正在发布'); }} /> : null}
      {toast ? <Toast message={toast} onDone={() => setToast('')} /> : null}
    </div>
  );
}

function RouteDetail({ route, policies, onClose }: { route: GatewayRoute; policies: Policy[]; onClose: () => void }) {
  ...
  return (
    <Drawer title={route.name} description={`${route.host}${route.path}`} onClose={onClose} width="wide">
      <div className="drawer-actions"><TypeBadge type={route.type} /><StatusBadge state={route.state} /></div>
...
function CreateRoute({ initial, gateways, services, onClose, onSave }: { initial?: GatewayRoute; gateways: Gateway[]; services: Service[]; onClose: () => void; onSave: (route: GatewayRoute) => void }) {
  const [type, setType] = useState<TrafficType>(initial?.type ?? 'API');
  const [name, setName] = useState(initial?.name ?? '新路由');
  const [gatewayID, setGatewayID] = useState(initial?.gatewayID ?? gateways[0]?.id ?? '');
  const [host, setHost] = useState(initial?.host ?? (gateways[0] ? gatewayDomains(gateways[0])[0] ?? '' : ''));
  const [path, setPath] = useState(initial?.path ?? '/new');
  const [method, setMethod] = useState(initial?.type === 'API' ? initial.match.split(' ')[0] ?? 'ANY' : 'ANY');
  const [matchMode, setMatchMode] = useState<'精确匹配' | '前缀匹配'>(initial?.type === 'API' && !initial.match.endsWith('/*') ? '精确匹配' : '前缀匹配');
  const [accessMode, setAccessMode] = useState<'需要调用方密钥' | '公开访问'>(initial?.accessMode ?? '需要调用方密钥');
  const [serviceID, setServiceID] = useState(initial?.targets[0]?.serviceID ?? '');
  const [selectedTools, setSelectedTools] = useState<string[]>(initial?.type === 'MCP' ? initial.published : []);
  const [secondLineEnabled, setSecondLineEnabled] = useState(Boolean(initial && initial.type !== 'AI' && initial.targets.length > 1));
  const [secondServiceID, setSecondServiceID] = useState(initial?.targets[1]?.serviceID ?? '');
  const [strategy, setStrategy] = useState<'主备切换' | '权重分流'>(initial?.forwarding.strategy === '权重分流' ? '权重分流' : '主备切换');
  const [primaryWeight, setPrimaryWeight] = useState(initial?.targets[0]?.weight ?? 80);
  const [modelMappings, setModelMappings] = useState<ModelMappingDraft[]>(initial?.type === 'AI' ? modelMappingsFromRoute(initial) : [newModelMapping(services)]);
  const [timeout, setTimeout] = useState(initial?.forwarding.timeout.replace(/\D/g, '') || '30');
  const [retries, setRetries] = useState(String(initial?.forwarding.retries ?? 0));
  const [pathHandling, setPathHandling] = useState(initial?.forwarding.pathHandling ?? '保持原路径');
  const [hostRewrite, setHostRewrite] = useState(initial?.forwarding.hostRewrite ?? '使用服务地址');
  ...
  const availableServices = services.filter((service) => service.state === 'healthy' || service.state === 'warning');
  const compatible = availableServices.filter((service) => service.type === (type === 'API' ? 'HTTP' : 'MCP'));
  ...
  const normalizedMappings = modelMappings.map((mapping) => normalizeModelMapping(mapping, availableServices));
  ...
  const changeType = (next: TrafficType) => {
    ...
    setTimeout(next === 'AI' ? '120' : '30'); setRetries('0');
  ...
  const save = () => {
    ...
    onSave({ id: initial?.id ?? `route-${Date.now()}`, name, type, gatewayID: gateway.id, gatewayName: gateway.name, host: effectiveHost, path, accessMode, match: type === 'API' ? apiCapability : type === 'AI' ? 'OpenAI API · 请求体 model' : 'Streamable HTTP · tools/call', published, targets, forwarding: { strategy: forwardingStrategy, timeout: `${timeout} 秒`, retries: Number(retries), pathHandling, hostRewrite }, requests: initial?.requests ?? '0', successRate: initial?.successRate ?? '—', latency: initial?.latency ?? '—', state: initial?.state ?? 'healthy' });
  };

  return (
    <Drawer title={initial ? '编辑路由' : '创建路由'} description="定义外部请求如何匹配并转发到服务" onClose={onClose} width="wide">
      <form onSubmit={(event) => submitForm(event, save)}>
        ...
        <FormActions submitLabel={initial ? '保存修改' : '创建路由'} submitDisabled={!routeReady} onCancel={onClose} />
      </form>
    </Drawer>
  );
}
...
function modelMappingsFromRoute(route: GatewayRoute): ModelMappingDraft[] {
  return route.published.map((published) => {
    const targets = route.targets.filter((target) => target.publishedCapability === published);
    const primary = targets.find((target) => target.role === '主线路') ?? targets[0];
    const backup = targets.find((target) => target.role === '备用线路');
    return { id: crypto.randomUUID(), published, primaryServiceID: primary?.serviceID ?? '', primaryModel: primary?.detail ?? '', backupEnabled: Boolean(backup), backupServiceID: backup?.serviceID ?? '', backupModel: backup?.detail ?? '' };
  });
}

function normalizeModelMapping(mapping: ModelMappingDraft, services: Service[]) {
  const modelServices = services.filter((service) => service.type === 'MODEL' && (service.state === 'healthy' || service.state === 'warning'));
...
export function ServicePage() {
  const { services, routes, certificates, addService, updateService, deleteService, updateServiceCredential } = usePrototype();
  ...
  const warnings = services.filter((item) => item.state === 'warning' || item.state === 'error' || item.state === 'unverified').length;
  return <div className="page-stack page-enter"><PageHeader eyebrow="流量管理" title="服务" description="HTTP、模型与 MCP 服务连接" actions={<PrimaryButton onClick={() => setCreating(true)}><Plus />创建服务</PrimaryButton>} /><section className="metric-grid four"><Metric label="HTTP 服务" value={String(services.filter((item) => item.type === 'HTTP').length)} note={`${healthyHTTPEndpoints} 个健康端点`} /><Metric label="模型服务" value={String(services.filter((item) => item.type === 'MODEL').length)} note={`${modelCount} 个实际模型`} /><Metric label="MCP 服务" value={String(services.filter((item) => item.type === 'MCP').length)} note={`${toolCount} 个已发现工具`} /><Metric label="运行告警" value={String(warnings)} note={warnings ? '服务或端点异常' : '全部服务正常'} tone={warnings ? 'warning' : 'good'} /></section><section className="card table-card"><header className="table-toolbar"><SearchField value={query} onChange={setQuery} placeholder="搜索服务、地址或能力" /><FilterTabs value={filter} onChange={setFilter} options={[{ value: 'ALL', label: '全部', count: services.length }, { value: 'HTTP', label: 'HTTP', count: services.filter((item) => item.type === 'HTTP').length }, { value: 'MODEL', label: '模型', count: services.filter((item) => item.type === 'MODEL').length }, { value: 'MCP', label: 'MCP', count: services.filter((item) => item.type === 'MCP').length }]} /></header><div className="table-head service-columns"><span>服务</span><span>协议与地址</span><span>端点 / 模型 / 工具</span><span>引用路由</span><span>成功率</span><span>关键性能</span><span>状态</span><span>操作</span></div>{visible.length ? visible.map((service) => <div key={service.id} className="table-row service-columns"><div className="name-cell"><TypeBadge type={service.type} /><div><strong>{service.name}</strong><small>{service.provider}</small></div></div><div><strong>{service.protocol}</strong><small>{service.endpoints[0]?.address}{service.endpoints.length > 1 ? ` 等 ${service.endpoints.length} 个` : ''}</small></div><div className="tag-cell">{(service.type === 'HTTP' ? service.endpoints.map((item) => item.address) : service.capabilities).slice(0, 3).map((item) => <code key={item}>{item}</code>)}</div><strong>{routes.filter((route) => route.targets.some((target) => target.serviceID === service.id)).length} 条</strong><span>{service.successRate}</span><span>{service.latency}</span><StatusBadge state={service.state} /><RowActions onDetail={() => setSelected(service)} onEdit={() => setEditing(service)} onDelete={() => setDeleting(service)} /></div>) : <EmptyState title="没有匹配的服务" description="请调整搜索或筛选条件。" />}</section>{selected ? <ServiceDetail service={services.find((service) => service.id === selected.id) ?? selected} routes={routes.filter((route) => route.targets.some((target) => target.serviceID === selected.id))} certificates={certificates} onRotateCredential={() => { setRotatingCredential(services.find((service) => service.id === selected.id) ?? selected); setSelected(null); }} onClose={() => setSelected(null)} /> : null}{creating ? <CreateService certificates={certificates} onClose={() => setCreating(false)} onSave={(service) => { addService(service); setCreating(false); setToast('服务已创建'); }} /> : null}{editing ? <CreateService initial={editing} certificates={certificates} onClose={() => setEditing(null)} onSave={(service) => { updateService(service); setEditing(null); setToast('服务修改已保存，正在发布'); }} /> : null}{rotatingCredential ? <RotateServiceCredential service={rotatingCredential} certificates={certificates} onClose={() => setRotatingCredential(null)} onSave={(clientCertificateID) => { updateServiceCredential(rotatingCredential.id, clientCertificateID); setRotatingCredential(null); setToast('服务凭据已更新'); }} /> : null}{deleting ? <DeleteConfirm resourceType="服务" resourceName={deleting.name} blockedReason={routes.some((route) => route.targets.some((target) => target.serviceID === deleting.id)) ? `还有 ${routes.filter((route) => route.targets.some((target) => target.serviceID === deleting.id)).length} 条路由使用该服务。请先修改这些路由的目标服务。` : undefined} onCancel={() => setDeleting(null)} onConfirm={() => { deleteService(deleting.id); setDeleting(null); setToast('服务已删除，正在发布'); }} /> : null}{toast ? <Toast message={toast} onDone={() => setToast('')} /> : null}</div>;
}
...
function CreateService({ initial, certificates, onClose, onSave }: { initial?: Service; certificates: Certificate[]; onClose: () => void; onSave: (service: Service) => void }) {
  const initialAuthentication = normalizedAuthentication(initial?.authentication);
  ...
  const [clientCertificateID, setClientCertificateID] = useState(initial?.clientCertificateID ?? '');
  const [transportSecurity, setTransportSecurity] = useState<Service['transportSecurity']>(initial?.transportSecurity ?? '明文连接');
  const [serverName, setServerName] = useState(initial?.serverName ?? '');
  const [trustMode, setTrustMode] = useState<'系统信任' | '指定信任证书'>(initial?.trustCertificateID ? '指定信任证书' : '系统信任');
  const [trustCertificateID, setTrustCertificateID] = useState(initial?.trustCertificateID ?? '');
  const [endpoints, setEndpoints] = useState<Array<{ address: string; weight: number }>>(initial?.endpoints.map(({ address, weight }) => ({ address, weight })) ?? [{ address: 'service.internal:8080', weight: 100 }]);
  const [loadBalancing, setLoadBalancing] = useState<Service['loadBalancing']>(initial?.loadBalancing ?? '轮询');
  const [healthMode, setHealthMode] = useState(initial?.healthCheck.startsWith('被动') ? '被动检查' : initial?.healthCheck.startsWith('TCP') ? 'TCP' : 'HTTP');
  const [healthPath, setHealthPath] = useState(initial?.healthCheck.match(/HTTP GET (\S+)/)?.[1] ?? '/health');
  const [healthInterval, setHealthInterval] = useState(initial?.healthCheck.match(/(\d+) 秒/)?.[1] ?? '10');
  const [models, setModels] = useState(initial?.type === 'MODEL' ? initial.capabilities.join(', ') : '');
  const [discoveredTools, setDiscoveredTools] = useState<string[]>(initial?.type === 'MCP' ? initial.capabilities : []);
  const [tested, setTested] = useState(Boolean(initial && initial.state !== 'unverified'));
  const protocolOptions = type === 'HTTP' ? ['HTTP/1.1', 'HTTP/2'] : type === 'MODEL' ? ['OpenAI 兼容 API', 'Anthropic Messages', 'AWS Bedrock'] : ['Streamable HTTP'];
  const authenticationOptions = type === 'HTTP' ? ['无认证', 'Bearer Token', 'Basic', 'mTLS'] : type === 'MODEL' ? ['API Key', 'Bearer Token', '自定义请求头', 'AWS 签名'] : ['无认证', 'Bearer Token', 'mTLS'];
  ...
  const testConnection = () => {
    if (type === 'MODEL' && !models.trim()) {
      setModels(protocol === 'Anthropic Messages' ? 'claude-sonnet-4, claude-haiku-4' : protocol === 'AWS Bedrock' ? 'anthropic.claude-sonnet-4, amazon.nova-pro' : 'qwen-max, qwen-plus');
    }
    if (type === 'MCP') setDiscoveredTools(['web_search', 'fetch_page', 'extract_text']);
    setTested(true);
  };
  const needsCredential = authentication !== '无认证';
  const canReuseStoredCredential = Boolean(initial && authentication === initialAuthentication && authentication !== '无认证');
  const credentialComplete = !needsCredential
    || (authentication === 'mTLS' ? Boolean(effectiveClientCertificateID)
      : canReuseStoredCredential || (authentication === 'Basic' || authentication === '自定义请求头' ? Boolean(credentialName.trim() && credential.trim())
  ...
  const canTest = endpoints.every((item) => item.address.trim()) && credentialComplete && tlsComplete;
  ...
  const save = () => onSave({
    id: initial?.id ?? `svc-${Date.now()}`, name, type, endpoints: endpointRecords, provider, protocol,
    ...
    loadBalancing, healthCheck, capabilities, successRate: initial?.successRate ?? '—', latency: initial?.latency ?? '—', state: tested ? 'healthy' : 'unverified',
  });

  return (
    <Drawer title={initial ? '编辑服务' : '创建服务'} description="连接、认证、TLS、端点与健康检查" onClose={onClose} width="wide">
      <form onSubmit={(event) => submitForm(event, save)}>
        ...
        <section className="form-section">
          ...
          <div className="form-grid">
            ...
            {authentication === 'Basic' ? <><label><span>用户名</span><input required={!canReuseStoredCredential} value={credentialName} onChange={(event) => { setCredentialName(event.target.value); setTested(false); }} placeholder={canReuseStoredCredential ? '留空表示继续使用已保存的用户名' : ''} /></label><label><span>密码</span><input required={!canReuseStoredCredential} type="password" value={credential} onChange={(event) => { setCredential(event.target.value); setTested(false); }} placeholder={canReuseStoredCredential ? '留空表示继续使用已保存的密码' : '保存后不再显示'} /></label></> : null}
            {authentication === '自定义请求头' ? <><label><span>请求头名称</span><input required={!canReuseStoredCredential} value={credentialName} onChange={(event) => { setCredentialName(event.target.value); setTested(false); }} placeholder={canReuseStoredCredential ? '留空表示继续使用已保存的名称' : '例如 x-api-key'} /></label><label><span>请求头值</span><input required={!canReuseStoredCredential} type="password" value={credential} onChange={(event) => { setCredential(event.target.value); setTested(false); }} placeholder={canReuseStoredCredential ? '留空表示继续使用已保存的值' : '保存后不再显示'} /></label></> : null}
            {authentication === 'AWS 签名' ? <><label><span>Access Key ID</span><input required={!canReuseStoredCredential} value={credentialName} onChange={(event) => { setCredentialName(event.target.value); setTested(false); }} placeholder={canReuseStoredCredential ? '留空表示继续使用已保存的 ID' : ''} /></label><label><span>Secret Access Key</span><input required={!canReuseStoredCredential} type="password" value={credential} onChange={(event) => { setCredential(event.target.value); setTested(false); }} placeholder={canReuseStoredCredential ? '留空表示继续使用已保存的密钥' : ''} /></label><label className="field-wide"><span>AWS 区域</span><input required={!canReuseStoredCredential} value={credentialRegion} onChange={(event) => { setCredentialRegion(event.target.value); setTested(false); }} placeholder={canReuseStoredCredential ? '留空表示继续使用已保存的区域' : '例如 us-east-1'} /></label></> : null}
            {authentication === 'Bearer Token' || authentication === 'API Key' ? <label className="field-wide"><span>{authentication}</span><input required={!canReuseStoredCredential} type="password" value={credential} onChange={(event) => { setCredential(event.target.value); setTested(false); }} placeholder={canReuseStoredCredential ? '留空表示继续使用已保存的凭据' : '保存后不再显示明文'} /></label> : null}
            {type === 'MODEL' ? <label className="field-wide"><span>实际模型（可选）</span><input value={models} onChange={(event) => { setModels(event.target.value); setTested(false); }} placeholder="测试连接后自动发现，也可手动填写" /><small>这是厂商接口中的真实名称；对外的稳定模型名在 AI 路由中设置</small></label> : null}
          </div>
        </section>
        <section className="form-section">
        ...
        <button className={`connection-test ${tested ? 'is-success' : ''}`} type="button" disabled={!canTest} onClick={testConnection}>{tested ? <CheckCircle2 /> : <CircleGauge />}<div><strong>{tested ? '连接验证通过' : type === 'MCP' ? '测试连接并发现工具' : type === 'MODEL' ? '测试连接并发现模型' : '测试连接'}</strong><span>{tested ? type === 'MCP' ? `已发现 ${discoveredTools.length} 个工具` : type === 'MODEL' ? `已发现 ${capabilities.length} 个实际模型` : `已验证 ${endpoints.length} 个端点、认证和 TLS` : '验证端点、认证、TLS 和协议兼容性'}</span></div></button>
        {tested && type !== 'HTTP' ? <div className="discovery-result"><strong>已发现{type === 'MODEL' ? '模型' : '工具'}</strong><div className="tag-cell">{capabilities.map((capability) => <code key={capability}>{capability}</code>)}</div></div> : null}
        {!tested ? <div className="form-note"><CircleGauge />可以先保存，但服务会标记为“待验证”，验证通过前不能被新路由选用。</div> : null}
        <FormActions submitLabel={initial ? '保存修改' : tested ? '保存服务' : '保存为待验证'} submitDisabled={!canTest || (endpoints.length > 1 && endpoints.reduce((sum, item) => sum + item.weight, 0) !== 100)} onCancel={onClose} />
      </form>
    </Drawer>
  );
}
...
export function CertificatePage() {
  const { certificates, gateways, services, addCertificate, updateCertificate, deleteCertificate } = usePrototype();
  ...
  return (
    <div className="page-stack page-enter">
      ...
      {creating ? <CreateCertificate onClose={() => setCreating(false)} onSave={(certificate) => { addCertificate(certificate); setCreating(false); setToast('证书已导入，正在发布'); }} /> : null}
      {editing ? <CreateCertificate initial={editing} onClose={() => setEditing(null)} onSave={(certificate) => { updateCertificate(certificate); setEditing(null); setToast('证书已更新，正在发布'); }} /> : null}
      {deleting ? <DeleteConfirm resourceType="证书" resourceName={deleting.name} blockedReason={referencesFor(deleting.id).length ? `该证书仍被 ${referencesFor(deleting.id).map((reference) => reference.name).join('、')} 引用。请先解除引用。` : undefined} onCancel={() => setDeleting(null)} onConfirm={() => { deleteCertificate(deleting.id); setDeleting(null); setToast('证书已删除，正在发布'); }} /> : null}
      {toast ? <Toast message={toast} onDone={() => setToast('')} /> : null}
    </div>
  );
}

function CreateCertificate({ initial, onClose, onSave }: { initial?: Certificate; onClose: () => void; onSave: (certificate: Certificate) => void }) {
  const [name, setName] = useState(initial?.name ?? '');
  const [intendedUsage, setIntendedUsage] = useState<Certificate['usage']>(initial?.usage ?? '服务器证书');
  ...
  const validPaste = certificatePEM.includes('-----BEGIN CERTIFICATE-----')
    && certificatePEM.includes('-----END CERTIFICATE-----')
    && (!requiresPrivateKey || privateKeyPEM.includes('-----BEGIN PRIVATE KEY-----') || privateKeyPEM.includes('-----BEGIN RSA PRIVATE KEY-----'));
  const validFiles = Boolean(certificateFileName) && (!requiresPrivateKey || Boolean(privateKeyFileName));
  ...
  return (
    <Drawer title={initial ? '更新证书' : '导入证书'} description={initial ? '上传新证书内容，资源引用将保持不变' : '系统读取用途、标识、签发机构和有效期'} onClose={onClose} width="wide">
      <form onSubmit={(event) => submitForm(event, save)}>
        <div className="form-grid">
          <label><span>证书名称</span><input required value={name} onChange={(event) => setName(event.target.value)} placeholder="例如 example.com 生产证书" /></label>
          <label><span>用于</span><select value={intendedUsage} onChange={(event) => { setIntendedUsage(event.target.value as Certificate['usage']); resetParsing(); }}><option>服务器证书</option><option>客户端证书</option><option>信任证书</option></select><small>解析时会校验证书是否符合这个用途</small></label>
        </div>
        <div className="input-method-selector" role="tablist" aria-label="证书输入方式">
          <button type="button" className={inputMode === 'file' ? 'is-selected' : ''} onClick={() => { setInputMode('file'); resetParsing(); }}>上传文件</button>
          <button type="button" className={inputMode === 'paste' ? 'is-selected' : ''} onClick={() => { setInputMode('paste'); resetParsing(); }}>手动粘贴</button>
        </div>
        {inputMode === 'file' ? (
          <div className="form-grid certificate-files">
            <label className="field-wide"><span>证书链文件</span><input required type="file" accept=".pem,.crt,.cer" onChange={(event) => { setCertificateFileName(event.target.files?.[0]?.name ?? ''); resetParsing(); }} /><small>{certificateFileName || '支持 PEM、CRT 或 CER'}</small></label>
            {requiresPrivateKey ? <label className="field-wide"><span>私钥文件</span><input required type="file" accept=".pem,.key" onChange={(event) => { setPrivateKeyFileName(event.target.files?.[0]?.name ?? ''); resetParsing(); }} /><small>{privateKeyFileName || '支持 PEM 或 KEY，不会再次显示内容'}</small></label> : null}
          </div>
        ) : (
          <div className="form-grid">
            <label className="field-wide"><span>证书链 PEM</span><textarea required value={certificatePEM} onChange={(event) => { setCertificatePEM(event.target.value); resetParsing(); }} placeholder="-----BEGIN CERTIFICATE-----" /></label>
            {requiresPrivateKey ? <label className="field-wide"><span>私钥 PEM</span><textarea required value={privateKeyPEM} onChange={(event) => { setPrivateKeyPEM(event.target.value); resetParsing(); }} placeholder="-----BEGIN PRIVATE KEY-----" /></label> : null}
          </div>
        )}
        <button className={`connection-test ${parsed ? 'is-success' : ''}`} type="button" disabled={!canParse} onClick={() => setParsed(true)}>
          {parsed ? <CheckCircle2 /> : <CircleGauge />}
          <div><strong>{parsed ? requiresPrivateKey ? '证书与私钥匹配' : '信任证书校验通过' : '解析并校验证书'}</strong><span>{parsed ? '用途、标识、签发机构和有效期已读取' : '保存前检查格式、用途、有效期和私钥匹配关系'}</span></div>
        </button>
        {parsed ? <dl className="definition-list certificate-preview"><div><dt>用途</dt><dd>{intendedUsage}</dd></div><div><dt>{intendedUsage === '服务器证书' ? '覆盖域名' : intendedUsage === '客户端证书' ? '客户端标识' : '证书主体'}</dt><dd>{identities.map((identity) => <code key={identity}>{identity}</code>)}</dd></div><div><dt>签发机构</dt><dd>{issuer}</dd></div><div><dt>有效期</dt><dd>2026-08-12 至 2027-08-12</dd></div><div><dt>指纹</dt><dd><code>SHA256 · A8:31:7F:••:9C</code></dd></div></dl> : null}
        <FormActions submitLabel={initial ? '保存并替换' : '导入证书'} submitDisabled={!parsed || !canParse} onCancel={onClose} />
      </form>
    </Drawer>
  );
}
