...
Analytics 与 ClickHouse 的在线去重可以处理正常恢复窗口内的重复。很久以前的备份可能已经超出 ClickHouse 去重窗口；直接重灌还可能让 `AggregatingMergeTree` 和 `SummingMergeTree` 聚合重复累加。

当前项目不提供“旧 Kafka 备份 + 旧 WAL 备份”的一键恢复，也没有可直接执行的历史聚合重建工具。若事故确实要求恢复这类备份，应由维护者先确定恢复时间窗、隔离恢复目标、导出并按稳定 ID 去重明细，再重建该时间窗的聚合表；未经演练不要向在线 Topic 和在线 ClickHouse 直接回灌。换句话说，这属于需要单独方案和校验脚本的数据恢复工程，不是本运行手册承诺支持的日常操作。
...
- 正常在线恢复窗口内的重复由稳定 ID 收敛，没有重复累计聚合。

最后一项只适用于当前在线去重窗口内的常规回放。历史备份恢复必须使用事故方案中单独定义的去重与聚合校验条件。
