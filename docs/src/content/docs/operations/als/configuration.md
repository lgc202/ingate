...
完整计算方法见[容量与吞吐规划](../../../development/architecture/als/capacity/)。

## Kafka

`brokers` 是发现集，不要求列出集群全部 Broker。客户端通过元数据找到目标分区的 Leader。

`write_timeout` 是一批记录等待 Kafka 投递结果的近似上限。franz-go 只在发送前和收到 Produce 响应后检查该时限，内部退避可能让结果稍晚返回。设得过短会增加“Broker 可能已经写入，但客户端没收到确认”的不确定失败；设得过长会拖慢进入 WAL 的速度。ALS 允许不确定的幂等写在超时后返回，并把原批次转入 WAL，因此边界故障可能产生相同记录 ID 的重复消息。参数应结合跨机房延迟、Broker 负载和故障切换时间调整。
