...
可靠接收告警使用两组多窗口预算消耗规则：5 分钟与 1 小时同时超过 14.4 倍预算时报告 Critical；30 分钟与 6 小时同时超过 6 倍预算时报告 Warning。短窗口负责尽快发现突发故障，长窗口过滤瞬时噪声。新鲜度是系统状态 SLI，不使用包含重放重复项的 Kafka 接收计数。
...
- **验证：** 两个窗口都低于 6 倍预算消耗速率，且不再产生新的直接 Critical 告警。
...
- **恢复：** 恢复 Broker 与网络；若 Topic 契约不合规，修复副本数和 `min.insync.replicas` 后等待 TopicMonitor 刷新。
...
- **恢复：** 恢复缺失副本并等待 ISR 回归；不要通过降低确认等级绕过可靠性契约。
...
- **恢复：** 修复 Kafka 或配置后重启 ALS 重新评估队首；若确认 WAL 数据损坏，先保全 Volume，再按事故流程离线修复。
