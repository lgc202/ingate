...
RateLimitPolicy 为 Gateway 或 Route 设置固定窗口请求上限。用户只配置限流语义，不需要了解 Envoy descriptor、Redis Key 或内部服务地址。
...
`requests` 表示 `windowSeconds` 秒内允许的请求数。窗口与 Unix 时间边界对齐。例如 60 秒窗口内的请求共用一个计数器，到下一个分钟边界自动进入新窗口。

多个 Authz 实例通过 Redis 共享计数，Lua 脚本原子完成检查与递增。超限返回 `429 Too Many Requests` 和 `Retry-After`；Redis 不可用时返回 `503`，不会绕过已声明规则。
...
当前不提供单实例计数、滑动窗口、Burst、自定义失败策略或任意复合计数 Key。
