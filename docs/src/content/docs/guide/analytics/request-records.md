...
请求记录采用至少一次投递：故障恢复时允许出现重复，Analytics 按稳定记录 ID 做幂等入库。Envoy ALS 协议没有逐批确认，因此“请求已经完成”不等于“记录一定到达 Ingate ALS”；这条链路也不应单独作为逐笔守恒的财务账本。

部署与故障行为见 [ALS 运行模型](../../../operations/als/)。Kafka、WAL、记录 ID 与去重的实现见 [ALS 设计总览](../../../development/architecture/als/)。
