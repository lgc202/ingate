...
- 匹配规则只决定请求是否命中路由
- 后端服务只决定命中后转发到哪里
- 策略配置决定命中后如何治理、改写、保护和观测；保存时由 admin-api 拆成 Route 原生能力、Policy 绑定或 Plugin 绑定
...
控制台策略能力目录和保存请求必须使用稳定 `capability` 作为逻辑主键，`displayName` 只用于页面展示；Route 原生能力的 `source` 使用 `RouteNative`，不能用中文策略名驱动后端转换或编译逻辑。
...
| 路由 | 匹配规则、后端服务、策略配置三段分离；后端保存时拆成 Route 原生能力、Policy 绑定或 Plugin 绑定 | 把超时、重试、路径改写放进匹配条件；把所有策略都保存成一坨 JSON |
...
后端再把产品 DTO 翻译成内部声明式资源、Route 原生能力、策略绑定、插件绑定和数据面配置。控制台中文展示名不能进入 compiler 作为协议判断依据。
