...
路由负责把请求匹配、转发目标和策略串起来。

概念边界：

- 匹配规则只决定请求是否命中路由
- 后端服务只决定命中后转发到哪里
- 策略配置决定命中后如何治理、改写、保护和观测
...
策略不等于插件。策略是用户意图，底层可以由网关原生能力、策略资源或插件实现。
...
| 路由 | 匹配规则、后端服务、策略配置三段分离 | 把超时、重试、路径改写放进匹配条件 |
...
后端再把产品 DTO 翻译成内部声明式资源、策略绑定、插件绑定和数据面配置。
...
策略是用户视角，插件是运行实现。超时、重试、路径改写这类能力可以由网关原生能力实现；AI 内容安全、Token 统计、语义缓存、RAG、审计等能力更适合插件化实现。
