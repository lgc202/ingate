...
Ingate 已经具备 `RateLimitPolicy`、`PolicyBinding`、`RedisStore`、compiler、xDS target 和 managed rate-limit Wasm 插件的基础链路。当前已验证的能力是：

- `Local` 模式可通过 Envoy shared data 在单 Envoy 实例多 worker 间共享计数
- `RateLimitPolicy + PolicyBinding` 可以编译为 managed rate-limit 插件配置
- all-in-one 镜像可以加载插件，真实 Envoy 请求能返回 200 / 429
- xDS 层可以自动注入 managed rate-limit HTTP filter

但 `Global` 模式还没有完整闭环。当前插件只能把 global 规则转换成 `RedisCheck`，没有真正访问 Redis。标准 Proxy-Wasm Go SDK 不提供 HTTP filter 主动拨 Redis TCP 的能力，Higress 的 Redis 能力依赖它自己的 wrapper / host function，不能作为 Ingate 的默认运行时假设。
...
- Global 模式通过内置 Redis 执行器访问 Redis
...
- 执行器支持连接复用、热更新、关闭旧连接
- 执行器不能阻塞 Envoy worker 过久，所有远程调用都有硬超时

### xDS 集成

- 自动注入 managed rate-limit Wasm filter
- 自动注入插件调用执行器所需的 internal cluster
- 自动下发插件配置、RedisStore 引用和执行器协议配置
...
- 执行器测试覆盖 Redis standalone / sentinel / cluster
...
xDS target translator
  ...
  |
  +--> Managed RateLimit Plugin Config
  |
  +--> Managed RateLimit Executor Cluster
          |
          v
Envoy HTTP filter chain
  |
  v
managed-rate-limit.wasm
  |
...
因此，Redis 访问必须放到 Ingate 自己的数据面执行边界中。这个边界不是用户手动部署的外部服务，而是 Ingate 数据面内置治理执行器，由 xDS 自动注入访问 cluster。
...
- Wasm 插件保持轻量和稳定
- Redis 执行器可以用普通 Go 运行时和成熟 Redis client
- 后续可以统一承载鉴权、配额、AI token 预算等需要外部状态的治理能力

### 执行器是内置数据面组件，不是用户级外部服务

`ingate-rate-limit-executor` 是 Ingate 数据面的内部组件。部署形态由 RuntimeGroup 决定：
...
插件调用执行器使用 Ingate 自定义协议，协议必须版本化。建议使用 JSON 作为 Wasm 到执行器的传输格式，原因是：

- 当前插件已经使用 JSON 配置
- Go WASI 下 JSON 成本可接受
- 便于调试 config dump 和执行器日志
...
生成 RedisCheck 列表
  |
  v
DispatchHttpCall 到 ingate-rate-limit-executor
  |
  v
Pause 请求
  |
...
插件必须保证每个请求最多有一个未完成执行器调用。回调返回后必须清理请求上下文，避免泄漏。
...
- managed rate-limit Wasm HTTP filter
- Wasm plugin config
- executor cluster
- executor endpoint
- RedisStore 信息进入执行器配置，而不是进入用户可见 route

cluster 命名建议：

```text
ingate/managed/rate-limit/executor
```

插件内固定使用这个 cluster 名，xDS 保证 cluster 存在。没有 global 策略时可以不注入 executor cluster。
...
- executor 调用超时：按 policy failure policy
- executor 返回协议错误：按 policy failure policy
...
- Cluster MOVED / ASK：执行器内部处理，失败后返回 error
...
- Redis 密码只通过 `PasswordRef` / SecretRef 进入执行器
- 插件配置不包含密码明文
- Admin API 响应不返回密码明文
- 日志中 Redis 地址可以打印，用户名和密码不可打印
- TLS Redis 必须支持 server name
- 执行器内部接口只对 Envoy / 本机可达，不暴露公网
...
- xDS 无 NACK，Envoy 能加载 Wasm 和 executor cluster
