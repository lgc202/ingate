...
- 使用调用方访问模式的 Route 通过 Envoy `ext_authz` 调用 `ingate-authz` 校验访问密钥；命中 RateLimitPolicy 的公开 Route 也通过同一标准扩展执行限流，但不会因此要求访问密钥
...
| `ingate-authz` | API Server、Redis | 校验调用方访问密钥和 Route 授权，执行共享请求限流 |
...
| 请求限流固定窗口计数 | Redis | Authorization |
| 当前周期 Token 额度计数 | Redis | AI ExtProc |

Redis 是安装级系统组件，不建模为用户资源。Authorization 使用 Redis 执行 RateLimitPolicy 的固定窗口计数，AI ExtProc 使用 Redis 保存 TokenQuotaPolicy 的实时计数；策略配置本身仍由 API Server 持久化到 etcd。完整执行语义见 [请求限流原理](resources/rate-limit-policy.md) 和 [Token 额度原理](resources/token-quota-policy.md)。

请求明细和模型线路调用按 Analytics 配置的保留时间由 ClickHouse TTL 自动清理；分钟流量聚合保留更长时间。`model_usage_1m` 是 Caller、Access Key、Route 和模型维度的累计用量账本，不跟随明细过期。
