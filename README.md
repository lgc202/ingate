...
- **运维助手后端**：以单进程运行 HTTP 服务和 Temporal Worker，并提供真实依赖状态 API
...
4. **助手链路**：单个 Assistant 进程承载 Kratos HTTP 与 Temporal Worker，并检查 MySQL、Temporal 和模型端点；Console 页面尚未接入
...
| [运维助手](https://lgc202.github.io/ingate/operations/assistant/) | 后端单进程骨架、依赖检查和优雅退出 |
...
Ingate 当前已实现 API 转发、AI 模型发布、调用方授权、流量治理、插件管理、请求分析，以及运维助手的新运行骨架。项目仍处于 `0.x` 快速演进阶段，资源协议和部署方式在 `1.0` 前可能调整。

以下能力不在当前范围：

- 运维助手对话、诊断、审批和执行，以及资源更新与删除、Route 与模型 Service 创建
- 用户自定义 MCP 工具、多 Agent 编排和定时自动化
