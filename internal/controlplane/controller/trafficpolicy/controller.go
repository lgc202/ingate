...
func NewController(ctx *controllerruntime.Context) *Controller {
	...
	if ctx.InformerFactory != nil {
		informer = ctx.InformerFactory.Policy().V1alpha1().TrafficPolicies().Informer()
	}
	return &Controller{informer: informer, index: ctx.Index, queue: ctx.GatewayQueue}
}

func (c *Controller) Name() string { return names.TrafficPolicyControllerName }
...
func (c *Controller) onAdd(obj interface{}) {
	policy, ok := asTrafficPolicy(obj)
	if !ok || c.index == nil {
		return
	}
	c.index.UpsertTrafficPolicy(policy)
	c.enqueue(c.index.AffectedGatewaysForTrafficPolicy(objectKeyOf(policy)))
}

func (c *Controller) onUpdate(oldObj, newObj interface{}) {
	oldPolicy, ok := asTrafficPolicy(oldObj)
	if !ok || c.index == nil {
		return
	}
	newPolicy, ok := asTrafficPolicy(newObj)
	if !ok {
		return
	}
	oldKey := objectKeyOf(oldPolicy)
	oldAffected := c.index.AffectedGatewaysForTrafficPolicy(oldKey)
	c.index.UpsertTrafficPolicy(newPolicy)
	newAffected := c.index.AffectedGatewaysForTrafficPolicy(objectKeyOf(newPolicy))
...
func (c *Controller) onDelete(obj interface{}) {
	...
	oldAffected := c.index.AffectedGatewaysForTrafficPolicy(key)
	c.index.DeleteTrafficPolicy(key)
...
func asTrafficPolicy(obj interface{}) (*policyv1alpha1.TrafficPolicy, bool) {
