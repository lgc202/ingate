...
func ValidateResourceID(value string, path *field.Path) field.ErrorList {
	if apivalidation.IsCanonicalID(value) {
		return nil
	}
...
func ValidateDisplayName(value string, path *field.Path) field.ErrorList {
	...
	case len(value) > apivalidation.MaxDisplayNameBytes:
		return field.ErrorList{field.TooLong(path, value, apivalidation.MaxDisplayNameBytes)}
	case !apivalidation.IsValidDisplayName(value):
