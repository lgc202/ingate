...
// strategy 定义 PolicyBinding 资源在 apiserver 存储前后的处理规则
type strategy struct {
	runtime.ObjectTyper
	names.NameGenerator
}

// statusStrategy 定义 PolicyBinding status 子资源更新规则
...
func (strategy) PrepareForCreate(ctx context.Context, obj runtime.Object) {
	binding := obj.(*gatewayv1.PolicyBinding)
	binding.Status = gatewayv1.ResourceStatus{}
...
func (strategy) PrepareForUpdate(ctx context.Context, obj, old runtime.Object) {
	newBinding := obj.(*gatewayv1.PolicyBinding)
	oldBinding := old.(*gatewayv1.PolicyBinding)
