...
func validatePolicy(policy *resource.MockResponsePolicy) field.ErrorList {
	...
	if !apivalidation.IsValidStatusCode(policy.Spec.StatusCode) {
		errs = append(errs, field.Invalid(
			specPath.Child("statusCode"),
			policy.Spec.StatusCode,
			fmt.Sprintf(
				"statusCode must be between %d and %d",
				apivalidation.MinStatusCode,
				apivalidation.MaxStatusCode,
			),
		))
	}
	normalizedContentType, valid := apivalidation.NormalizeContentType(policy.Spec.ContentType)
	...
	if len(policy.Spec.Body) > apivalidation.MaxBodyBytes {
		errs = append(errs, field.Invalid(
			specPath.Child("body"),
...
func validateHeaders(headers []resource.HeaderValue, path *field.Path) field.ErrorList {
	...
	if len(headers) > apivalidation.MaxHeaders {
		errs = append(errs, field.TooMany(path, len(headers), apivalidation.MaxHeaders))
		headers = headers[:apivalidation.MaxHeaders]
	}
