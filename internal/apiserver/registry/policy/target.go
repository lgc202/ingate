// Package policy 提供不同策略资源共享的 API Server 存储规则。
package policy
...
// ValidateTargets 校验具体策略允许的目标类型、名称和重复引用。
func ValidateTargets(
	...
	if len(refs) > apivalidation.MaxTargets {
		errs = append(errs, field.TooMany(path, len(refs), apivalidation.MaxTargets))
		refs = refs[:apivalidation.MaxTargets]
	}
	...
	for i, ref := range refs {
		...
		if !apivalidation.IsCanonicalID(ref.Name) {
...
// CanonicalizeTargets 规范化目标 ID，并按稳定身份排序。
func CanonicalizeTargets(refs []resource.PolicyTargetRef) {
	for i := range refs {
		if targetID, valid := apivalidation.NormalizeID(refs[i].Name); valid {
