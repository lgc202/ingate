...
// strategy 定义 AIPolicy 资源在 apiserver 存储前后的处理规则
type strategy struct {
	runtime.ObjectTyper
	names.NameGenerator
}

// statusStrategy 定义 AIPolicy status 子资源更新规则
...
func (strategy) PrepareForCreate(ctx context.Context, obj runtime.Object) {
	policy := obj.(*gatewayv1.AIPolicy)
	policy.Status = gatewayv1.ResourceStatus{}
	policy.Generation = 1
}
...
func (strategy) PrepareForUpdate(ctx context.Context, obj, old runtime.Object) {
	newPolicy := obj.(*gatewayv1.AIPolicy)
	oldPolicy := old.(*gatewayv1.AIPolicy)
