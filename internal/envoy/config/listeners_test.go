...
func TestCompilerBuildsSharedHTTPSListenerWithGatewayCertificates(t *testing.T) {
	...
	if result.HasErrors() {
		t.Fatalf("Compiler.Compile(shared HTTPS listener) diagnostics = %v, want no errors", result.Diagnostics)
	}
	if got := len(result.Config.Listeners); got != 1 {
		t.Fatalf("Compiler.Compile(shared HTTPS listener) listener count = %d, want 1", got)
	}

	listener := result.Config.Listeners[0]
	if got, want := listener.Name, "ingate/https-8443"; got != want {
		t.Errorf("Compiler.Compile(shared HTTPS listener) listener name = %q, want %q", got, want)
	}
	if got := len(listener.ListenerFilters); got != 1 {
		t.Errorf("Compiler.Compile(shared HTTPS listener) listener filter count = %d, want 1", got)
	} else if got, want := listener.ListenerFilters[0].Name, tlsInspectorFilterName; got != want {
		t.Errorf("Compiler.Compile(shared HTTPS listener) listener filter name = %q, want %q", got, want)
	}
	if listener.DefaultFilterChain != nil {
		t.Errorf("Compiler.Compile(shared HTTPS listener) default filter chain = %v, want nil", listener.DefaultFilterChain)
	}
	if got := len(listener.FilterChains); got != 2 {
		t.Fatalf("Compiler.Compile(shared HTTPS listener) filter chain count = %d, want 2", got)
	}
	...
	for _, filterChain := range listener.FilterChains {
		if filterChain.FilterChainMatch == nil || len(filterChain.FilterChainMatch.ServerNames) != 1 {
			t.Errorf("Compiler.Compile(shared HTTPS listener) filter chain match = %v, want one SNI server name", filterChain.FilterChainMatch)
			continue
		}
		hostname := filterChain.FilterChainMatch.ServerNames[0]
		wantCertificate := wantCertificates[hostname]
		if wantCertificate == nil {
			t.Errorf("Compiler.Compile(shared HTTPS listener) SNI hostname = %q, want api.example.com or mcp.example.com", hostname)
			continue
		}
		if got, want := filterChain.FilterChainMatch.TransportProtocol, tlsTransportProtocol; got != want {
			t.Errorf("Compiler.Compile(shared HTTPS listener) transport protocol for %q = %q, want %q", hostname, got, want)
		}
		if filterChain.TransportSocket == nil {
			t.Errorf("Compiler.Compile(shared HTTPS listener) transport socket for %q = nil, want TLS", hostname)
			continue
		}
		if got, want := filterChain.TransportSocket.Name, tlsTransportSocketName; got != want {
			t.Errorf("Compiler.Compile(shared HTTPS listener) transport socket for %q = %q, want %q", hostname, got, want)
		}

		tlsContext := &tlsv3.DownstreamTlsContext{}
		if err := filterChain.TransportSocket.GetTypedConfig().UnmarshalTo(tlsContext); err != nil {
			t.Fatalf("Compiler.Compile(shared HTTPS listener) TLS context for %q could not be decoded: %v", hostname, err)
		}
		tlsCertificates := tlsContext.GetCommonTlsContext().GetTlsCertificates()
		if got := len(tlsCertificates); got != 1 {
			t.Fatalf("Compiler.Compile(shared HTTPS listener) TLS certificate count for %q = %d, want 1", hostname, got)
		}
		if got, want := tlsCertificates[0].GetCertificateChain().GetInlineString(), wantCertificate.Spec.CertificatePEM; got != want {
			t.Errorf("Compiler.Compile(shared HTTPS listener) certificate PEM for %q does not match referenced certificate", hostname)
		}
		if got, want := tlsCertificates[0].GetPrivateKey().GetInlineString(), wantCertificate.Spec.PrivateKeyPEM; got != want {
			t.Errorf("Compiler.Compile(shared HTTPS listener) private key PEM for %q does not match referenced certificate", hostname)
		}
		if got := len(filterChain.Filters); got != 1 {
			t.Fatalf("Compiler.Compile(shared HTTPS listener) network filter count for %q = %d, want 1", hostname, got)
		}
		manager := &hcmv3.HttpConnectionManager{}
		if err := filterChain.Filters[0].GetTypedConfig().UnmarshalTo(manager); err != nil {
			t.Fatalf("Compiler.Compile(shared HTTPS listener) HCM for %q could not be decoded: %v", hostname, err)
		}
		if got, want := manager.GetRds().GetRouteConfigName(), "ingate/https-8443/routes"; got != want {
			t.Errorf("Compiler.Compile(shared HTTPS listener) RDS name for %q = %q, want %q", hostname, got, want)
		}
	}
}
...
func TestCompilerBuildsDefaultHTTPSFilterChainForCatchAllGateway(t *testing.T) {
	...
	if result.HasErrors() {
		t.Fatalf("Compiler.Compile(catch-all HTTPS listener) diagnostics = %v, want no errors", result.Diagnostics)
	}
	if got := len(result.Config.Listeners); got != 1 {
		t.Fatalf("Compiler.Compile(catch-all HTTPS listener) listener count = %d, want 1", got)
	}

	listener := result.Config.Listeners[0]
	if got := len(listener.FilterChains); got != 0 {
		t.Errorf("Compiler.Compile(catch-all HTTPS listener) matched filter chain count = %d, want 0", got)
	}
	if listener.DefaultFilterChain == nil {
		t.Fatal("Compiler.Compile(catch-all HTTPS listener) default filter chain = nil, want TLS filter chain")
	}
	if listener.DefaultFilterChain.FilterChainMatch != nil {
		t.Errorf("Compiler.Compile(catch-all HTTPS listener) default filter chain match = %v, want nil", listener.DefaultFilterChain.FilterChainMatch)
	}
}
...
func TestCompilerReportsInvalidHTTPSCertificateReferences(t *testing.T) {
	...
	if !result.HasErrors() {
		t.Fatal("Compiler.Compile(invalid HTTPS certificate references) has errors = false, want true")
	}
	...
	for _, want := range wants {
		if !containsDiagnostic(result.Diagnostics, want.Kind, want.ID, want.Reason) {
			t.Errorf(
				"Compiler.Compile(invalid HTTPS certificate references) diagnostics = %v, want kind %q id %q reason %q",
				result.Diagnostics,
				want.Kind,
				want.ID,
				want.Reason,
...
func TestCompilerReportsConflictingHostOwnership(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if !result.HasErrors() {
				t.Fatalf("Compiler.Compile(%q) has errors = false, want true", tt.name)
			}
			for _, gateway := range []*gatewayv1.Gateway{firstGateway, secondGateway} {
				if !containsDiagnostic(result.Diagnostics, gatewayv1.KindGateway, gateway.Name, ReasonConflict) {
					t.Errorf(
						"Compiler.Compile(%q) diagnostics = %v, want conflict for gateway %q",
						tt.name,
						result.Diagnostics,
						gateway.Name,
					)
...
func TestCompilerReportsDifferentProtocolsOnSamePort(t *testing.T) {
	...
	if !result.HasErrors() {
		t.Fatal("Compiler.Compile(different protocols on same port) has errors = false, want true")
	}
	for _, gateway := range []*gatewayv1.Gateway{httpGateway, httpsGateway} {
		if !containsDiagnostic(result.Diagnostics, gatewayv1.KindGateway, gateway.Name, ReasonConflict) {
			t.Errorf(
				"Compiler.Compile(different protocols on same port) diagnostics = %v, want conflict for gateway %q",
				result.Diagnostics,
				gateway.Name,
			)
		}
...
func TestCompilerAllowsSameHostnameOnHTTPAndHTTPSDefaultPorts(t *testing.T) {
	...
	if result.HasErrors() {
		t.Fatalf("Compiler.Compile(same hostname on HTTP and HTTPS default ports) diagnostics = %v, want no errors", result.Diagnostics)
	}
	if got := len(result.Config.Listeners); got != 2 {
		t.Errorf("Compiler.Compile(same hostname on HTTP and HTTPS default ports) listener count = %d, want 2", got)
	}
}
