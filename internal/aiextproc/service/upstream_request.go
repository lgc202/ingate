...
// 上游 filter 无需再次接收 Body，直接用入口阶段保存的原文返回 CONTINUE_AND_REPLACE
func (s *streamState) handleUpstreamHeaders(
	headers *corev3.HeaderMap,
	attributes map[string]*structpb.Struct,
) (*extprocv3.ProcessingResponse, error) {
	s.upstream = true
	// 关联 Header 由入口 filter 注入，并随 Envoy 内部转发保留到上游 filter
	s.requestID = headerValue(headers, filterconfig.RequestIDHeader)
	if s.requestID == "" {
		return nil, errors.New("AI upstream request ID is missing")
	}
	request, ok := s.service.findRequest(s.requestID)
	if !ok {
	...
	selected, err := filterconfig.FromAttributes(
		attributes,
		headerValue(headers, filterconfig.UpstreamModelHeader),
	)
	if err != nil {
		return nil, err
	}
	credentials, err := s.service.models.Get(selected.ID)
	if err != nil {
		return nil, err
	}
	// 每次重试都读取入口原文，不能基于上一条线路已经转换过的 Body 再次转换
	...
	// 入口响应阶段需要知道最终线路协议，重试还需要恢复基于入口原文生成的 Body
	retry := request.startUpstreamAttempt(selected)

	replaceBody := converted.BodyChanged || retry
	headerMutation := upstreamHeaderMutation(selected, credentials.APIKey, len(converted.Body), replaceBody)
	if !replaceBody {
	...
	// 重试必须从入口原文重新生成 Body，不能复用上一条模型线路的协议格式
...
func convertUpstreamRequest(
	body []byte,
	selected filterconfig.SelectedService,
