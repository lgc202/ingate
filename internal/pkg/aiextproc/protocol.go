...
const (
	...
	// TokenQuotaExceededResponseDetails 通过 Envoy response_code_details 标记额度拒绝。
	TokenQuotaExceededResponseDetails = "ingate_ai_token_quota_exceeded"
	...
	ServiceIDAttribute = "xds.upstream_host_metadata.filter_metadata" +
		"['" + MetadataNamespace + "']['" + ServiceIDField + "']"
	// ServiceProtocolAttribute 读取模型 Service 使用的厂商协议。
	ServiceProtocolAttribute = "xds.upstream_host_metadata.filter_metadata" +
...
func IsInternalHeader(name string) bool {
	return strings.EqualFold(name, RequestIDHeader) ||
		strings.EqualFold(name, ModelHeader) ||
		strings.EqualFold(name, UpstreamModelHeader)
}
