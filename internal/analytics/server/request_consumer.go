...
func (c *RequestConsumer) Start(ctx context.Context) error {
	...
	for {
		...
		decoded := decodeRequestRecords(messages)
		if decoded.invalidCount > 0 {
			// 无法解析的消息不可能通过重试恢复，提交其 offset 防止毒消息永久阻塞分区。
			c.invalid.Add(uint64(decoded.invalidCount))
			c.logger.WarnContext(
				runCtx,
				"invalid request record messages discarded",
				"err", decoded.firstInvalidErr,
				"count", decoded.invalidCount,
			)
		}
