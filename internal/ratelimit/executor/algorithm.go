...
func fixedWindow(ctx context.Context, client redis.UniversalClient, check protocol.Check) (protocol.Result, error) {
	...
	if err != nil {
		return protocol.Result{}, fmt.Errorf("execute fixed window script: %w", err)
	}
	if len(values) < 2 {
		return protocol.Result{}, fmt.Errorf("fixed window script returned %d values", len(values))
	}
...
func slidingWindow(ctx context.Context, client redis.UniversalClient, check protocol.Check) (protocol.Result, error) {
	windowMillis := check.Limit.WindowSeconds * 1000
	...
	if err != nil {
		return protocol.Result{}, fmt.Errorf("execute sliding window script: %w", err)
	}
	if len(values) < 3 {
		return protocol.Result{}, fmt.Errorf("sliding window script returned %d values", len(values))
	}
...
func tokenBucket(ctx context.Context, client redis.UniversalClient, check protocol.Check) (protocol.Result, error) {
	windowMillis := check.Limit.WindowSeconds * 1000
	...
	if err != nil {
		return protocol.Result{}, fmt.Errorf("execute token bucket script: %w", err)
	}
	if len(values) < 5 {
		return protocol.Result{}, fmt.Errorf("token bucket script returned %d values", len(values))
	}
...
func result(check protocol.Check, allowed bool, current, limit, reset int) protocol.Result {
