// Package accesskeyindex 将访问密钥发布为 AI 数据面可直接查询的 Redis 索引
package accesskeyindex
...
func (i *Index) Reconcile(ctx context.Context, credentials []Credential) error {
...
func (i *Index) Save(ctx context.Context, credential Credential) error {
...
func (i *Index) Rotate(ctx context.Context, previousHash [32]byte, credential Credential) error {
...
func (i *Index) Delete(ctx context.Context, credential Credential) error {
	key := sharedaccesskey.CredentialKey(credential.SecretHash)
	_, err := i.redis.TxPipelined(ctx, func(pipe redis.Pipeliner) error {
		pipe.Del(ctx, key)
		pipe.SRem(ctx, sharedaccesskey.CredentialSetKey, key)
...
func (i *Index) LastUsed(ctx context.Context, accessKeyIDs []string) (map[string]time.Time, error) {
