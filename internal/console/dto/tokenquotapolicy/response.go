...
import (
	...
	"github.com/lgc202/ingate/internal/console/service/resourcestatus"
...
func tokenQuotaPolicyFromResource(
	...
	return TokenQuotaPolicy{
		...
		Status:      consoledto.NewResourceStatus(status),
...
func tokenQuotaPolicyTargets(
	...
	targetNames policytarget.DisplayNames,
	disabled bool,
) []consoledto.PolicyTarget {
	targets := make([]consoledto.PolicyTarget, 0, len(policy.Spec.TargetRefs))
	for _, ref := range policy.Spec.TargetRefs {
		status := resourcestatus.ForPolicyTarget(policy.Generation, tokenQuotaTargetConditions(policy.Status.Targets, ref))
		if disabled {
			status = resourcestatus.Disabled()
		}
		targets = append(targets, consoledto.PolicyTarget{
			Kind:        consoledto.PolicyTargetKind(ref.Kind),
			ID:          ref.Name,
