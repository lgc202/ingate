...
func (s *Server) Run(ctx context.Context) error {
	...
	if err != nil {
		return fmt.Errorf("listen for HTTP on %q: %w", s.listenAddress, err)
	}
	...
	case <-ctx.Done():
		...
		if err := httpServer.Shutdown(shutdownCtx); err != nil {
			shutdownErr := fmt.Errorf("shut down HTTP: %w", err)
			if closeErr := httpServer.Close(); closeErr != nil {
				shutdownErr = errors.Join(shutdownErr, fmt.Errorf("force close HTTP: %w", closeErr))
			}
			if serveErr := <-serverErr; serveErr != nil && !errors.Is(serveErr, http.ErrServerClosed) {
				shutdownErr = errors.Join(shutdownErr, fmt.Errorf("serve HTTP: %w", serveErr))
			}
			return shutdownErr
		}
