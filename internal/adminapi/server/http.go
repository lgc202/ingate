...
func (s *Services) registerHTTP(server *kratoshttp.Server) {
	...
	adminv1.RegisterServiceManagementServiceHTTPServer(server, s.serviceManagement)
	...
	adminv1.RegisterRequestRecordServiceHTTPServer(server, s.requestRecord)
	adminv1.RegisterTrafficAnalysisServiceHTTPServer(server, s.trafficAnalysis)
