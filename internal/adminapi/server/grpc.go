...
func (s *Services) registerGRPC(server *kratosgrpc.Server) {
	...
	adminv1.RegisterServiceManagementServiceServer(server, s.serviceManagement)
	...
	adminv1.RegisterRequestRecordServiceServer(server, s.requestRecord)
	adminv1.RegisterTrafficAnalysisServiceServer(server, s.trafficAnalysis)
