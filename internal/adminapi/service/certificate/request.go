...
func parseCertificateSpec(
	...
	if displayName == "" {
		return resource.CertificateSpec{}, adminv1.ErrorInvalidArgument("证书名称不能为空")
	}
...
func parseCertificateReplacement(
	...
	if displayName == "" {
		return resource.CertificateSpec{}, false, adminv1.ErrorInvalidArgument("证书名称不能为空")
	}
	if (request.CertificatePem == nil) != (request.PrivateKeyPem == nil) {
		return resource.CertificateSpec{}, false, adminv1.ErrorInvalidArgument("替换证书时必须同时提供证书内容和私钥")
	}
...
func parseKeyPair(certificatePEM, privateKeyPEM string) (string, string, error) {
	...
	if certificatePEM == "" || privateKeyPEM == "" {
		return "", "", adminv1.ErrorInvalidArgument("证书内容和私钥不能为空")
	}
	if len(certificatePEM) > certificateutil.MaxCertificatePEMBytes {
		return "", "", adminv1.ErrorInvalidArgument("证书链大小不能超过 256 KiB")
	}
	if len(privateKeyPEM) > certificateutil.MaxPrivateKeyPEMBytes {
		return "", "", adminv1.ErrorInvalidArgument("私钥大小不能超过 64 KiB")
	}
	if _, err := certificateutil.ParseKeyPair(certificatePEM, privateKeyPEM); err != nil {
		return "", "", adminv1.ErrorInvalidArgument("证书内容与私钥格式不正确或不匹配").WithCause(err)
	}
