...
type Service struct {
	plugins *wasmbiz.Usecase
}

// NewService 创建 WasmPlugin 协议服务。
func NewService(plugins *wasmbiz.Usecase) *Service {
...
func (s *Service) ListWasmPlugins(
	...
	page, err := s.plugins.List(
		...
		conversion.ResourceFilter(request.GetQuery(), nil, request.GetState()),
	)
	if err != nil {
		return nil, err
	}
