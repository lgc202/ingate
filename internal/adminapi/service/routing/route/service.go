...
import (
	...
	routebiz "github.com/lgc202/ingate/internal/adminapi/biz/routing/route"
...
func (s *Service) ListRoutes(
	...
	filter := routebiz.ListFilter{
		Filter: conversion.ResourceFilter(
			request.GetQuery(),
			request.Enabled,
			request.GetState(),
		),
...
func (s *Service) CreateRoute(
	...
	spec, err := parseSpec(request.GetName(), request.GetConfig())
	if err != nil {
		return nil, err
	}
