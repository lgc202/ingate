...
func parseServiceTargets(targets []*adminv1.ServiceTarget) ([]resource.UpstreamRef, error) {
	...
	if len(targets) > apivalidation.MaxServiceTargets {
	...
	for i, target := range targets {
		...
		serviceID, valid := apivalidation.NormalizeID(target.GetServiceId())
		if !valid {
		...
		if weight < apivalidation.MinTargetWeight || weight > apivalidation.MaxTargetWeight {
...
func parseAIRoute(aiRoute *adminv1.AIRoute) (*resource.AIRoute, error) {
	...
	if len(modelInputs) > apivalidation.MaxAIModels {
	...
	for i, modelInput := range modelInputs {
		...
		if !apivalidation.IsValidModelName(modelName) || modelName != modelInput.GetName() {
...
func parseAIModelTargets(targets []*adminv1.AIModelTarget) ([]resource.AIModelTarget, error) {
	...
	for i, target := range targets {
		...
		if !apivalidation.IsValidModelName(actualModel) || actualModel != target.GetModel() {
