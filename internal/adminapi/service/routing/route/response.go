...
func routeResponse(route *resource.Route) *adminv1.Route {
	resourceStatus := status.ForEnabledResource(route.Generation, route.Spec.Enabled, route.Status.Conditions)
	...
	return &adminv1.Route{
		...
		CreatedAt: conversion.Timestamp(route.CreationTimestamp.Time),
		UpdatedAt: conversion.Timestamp(conversion.ResourceUpdatedAt(route.Annotations)),
	}
}
