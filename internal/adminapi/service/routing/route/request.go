...
func parseSpec(displayName string, config *adminv1.RouteConfig) (resource.RouteSpec, error) {
	displayName = strings.TrimSpace(displayName)
	if !apivalidation.IsValidDisplayName(displayName) {
		return resource.RouteSpec{}, adminv1.ErrorInvalidArgument("路由名称不能为空、不能包含控制字符且不能超过 256 字节")
	}
	if config == nil {
		return resource.RouteSpec{}, adminv1.ErrorInvalidArgument("路由配置不能为空")
	}
...
func validateSpec(spec resource.RouteSpec) error {
	requestTimeoutMillis := spec.Timeout.RequestMillis
	if requestTimeoutMillis == 0 {
		requestTimeoutMillis = apivalidation.DefaultRequestTimeoutMillis
	} else if requestTimeoutMillis < apivalidation.MinRequestTimeoutMillis ||
		requestTimeoutMillis > apivalidation.MaxRequestTimeoutMillis {
		return adminv1.ErrorInvalidArgument("请求总超时超出允许范围")
	}

	if spec.Retry != nil {
		if spec.Retry.Attempts < apivalidation.MinRetryAttempts ||
			spec.Retry.Attempts > apivalidation.MaxRetryAttempts {
			return adminv1.ErrorInvalidArgument("重试次数超出允许范围")
		}
		if spec.Retry.PerTryTimeoutMillis < apivalidation.MinPerTryTimeoutMillis ||
			...
			return adminv1.ErrorInvalidArgument("单次重试超时超出允许范围")
		}
		if spec.Retry.PerTryTimeoutMillis > requestTimeoutMillis {
			return adminv1.ErrorInvalidArgument("单次重试超时不能大于请求总超时")
		}
	}
	...
	if len(spec.Match.Methods) != 1 || spec.Match.Methods[0] != http.MethodPost {
		return adminv1.ErrorInvalidArgument("AI 路由目前只支持 POST 请求")
	}
	if usesReservedAIHeader(spec.Match.Headers, spec.RequestHeaderModifier) {
		return adminv1.ErrorInvalidArgument("AI 路由不能匹配或修改系统保留的 Header")
	}
	return nil
}
...
func parseAccessMode(mode adminv1.RouteAccessMode) (resource.RouteAccessMode, error) {
	...
	default:
		return "", adminv1.ErrorInvalidArgument("请选择访问方式")
	}
}
...
func parseHostRewrite(rewrite *adminv1.HostRewrite) (resource.HostRewrite, error) {
	...
	case adminv1.HostRewriteMode_HOST_REWRITE_MODE_SERVICE_HOST:
		if hostname != "" {
			return resource.HostRewrite{}, adminv1.ErrorInvalidArgument("使用服务端点主机名时不能填写自定义主机名")
		}
	...
	case adminv1.HostRewriteMode_HOST_REWRITE_MODE_PRESERVE:
		if hostname != "" {
			return resource.HostRewrite{}, adminv1.ErrorInvalidArgument("保留请求主机时不能填写自定义主机名")
		}
	...
	case adminv1.HostRewriteMode_HOST_REWRITE_MODE_CUSTOM:
		...
		if !ok || normalized == "*" {
			return resource.HostRewrite{}, adminv1.ErrorInvalidArgument("自定义主机名格式不正确")
		}
		return resource.HostRewrite{Mode: resource.HostRewriteCustom, Hostname: normalized}, nil
	default:
		return resource.HostRewrite{}, adminv1.ErrorInvalidArgument("请选择转发请求使用的主机名")
	}
}

func parseGatewayRefs(gatewayIDs []string) ([]string, error) {
	if len(gatewayIDs) == 0 {
		return nil, adminv1.ErrorInvalidArgument("至少需要选择一个网关")
	}
	if len(gatewayIDs) > apivalidation.MaxGatewayRefs {
		return nil, adminv1.ErrorInvalidArgument("关联网关数量超过限制")
	}
	...
	for i, rawGatewayID := range gatewayIDs {
		gatewayID, valid := apivalidation.NormalizeID(rawGatewayID)
		if !valid {
			return nil, adminv1.ErrorInvalidArgument("网关 ID 不正确")
		}
		if seenGatewayIDs[gatewayID] {
			return nil, adminv1.ErrorInvalidArgument("同一个网关只能选择一次")
		}
...
func parseHostnames(rawHostnames []string) ([]string, error) {
	if len(rawHostnames) > apivalidation.MaxHostnames {
		return nil, adminv1.ErrorInvalidArgument("路由域名数量超过限制")
	}
	...
	for i, rawHostname := range rawHostnames {
		...
		if !ok || hostname == "*" {
			return nil, adminv1.ErrorInvalidArgument("路由域名格式不正确，留空列表表示不限制域名")
		}
		if seenHostnames[hostname] {
			return nil, adminv1.ErrorInvalidArgument("路由域名不能重复")
		}
