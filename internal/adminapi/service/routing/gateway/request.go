...
func parseListeners(configs []*adminv1.GatewayListener) ([]resource.Listener, error) {
	...
	if len(configs) > apivalidation.MaxListeners {
	...
	for _, config := range configs {
		...
		if seenNames[listener.Name] {
			return nil, adminv1.ErrorInvalidArgument("监听入口名称 %q 不能重复", listener.Name)
		}
...
func parseListener(config *adminv1.GatewayListener) (resource.Listener, error) {
	...
	if !apivalidation.IsValidListenerName(listenerName) {
	...
	if !apivalidation.IsValidListenerPort(port) {
...
func checkListenerOverlap(listener resource.Listener, existing []resource.Listener) error {
	...
	for _, current := range existing {
		...
		if listener.Protocol != current.Protocol {
			return adminv1.ErrorInvalidArgument("端口 %d 不能同时用于 HTTP 和 HTTPS", listener.Port)
		}
		currentHostname := cmp.Or(current.Hostname, "*")
		if hostnameutil.Overlaps(hostname, currentHostname) {
			return adminv1.ErrorInvalidArgument("端口 %d 上的监听域名范围不能重叠", listener.Port)
		}
	}
	return nil
}
