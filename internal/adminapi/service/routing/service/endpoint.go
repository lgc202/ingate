...
func parseEndpoints(configs []*adminv1.ServiceEndpoint) ([]resource.Endpoint, error) {
	...
	if len(configs) > apivalidation.MaxEndpoints {
	...
	for _, config := range configs {
		...
		if seenEndpointKeys[endpointKey] {
			return nil, adminv1.ErrorInvalidArgument("服务端点 %q 不能重复", endpointKey)
		}
...
func parseEndpoint(config *adminv1.ServiceEndpoint) (resource.Endpoint, error) {
	address := apivalidation.NormalizeAddress(config.GetAddress())
	if !apivalidation.IsValidAddress(address) {
		return resource.Endpoint{}, adminv1.ErrorInvalidArgument("服务端点地址格式不正确")
	}
	port := int(config.GetPort())
	if !apivalidation.IsValidEndpointPort(port) {
		return resource.Endpoint{}, adminv1.ErrorInvalidArgument("服务端点端口必须在 1 到 65535 之间")
	}
	weight := cmp.Or(int(config.GetWeight()), apivalidation.DefaultEndpointWeight)
	if !apivalidation.IsValidEndpointWeight(weight) {
