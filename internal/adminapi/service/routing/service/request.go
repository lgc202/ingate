...
func parseSpec(
	displayName string,
	endpointConfigs []*adminv1.ServiceEndpoint,
	tlsConfig *adminv1.ServiceTLS,
	loadBalancingConfig adminv1.LoadBalancingPolicy,
	healthCheckConfig *adminv1.ServiceHealthCheck,
) (resource.UpstreamSpec, error) {
	displayName = strings.TrimSpace(displayName)
	if displayName == "" {
		return resource.UpstreamSpec{}, adminv1.ErrorInvalidArgument("服务名称不能为空")
	}
...
func parseTLS(config *adminv1.ServiceTLS) (*resource.UpstreamTLS, error) {
	if config == nil {
		return nil, nil
	}
	serverName := apivalidation.NormalizeAddress(config.GetServerName())
	if !apivalidation.IsValidAddress(serverName) {
		return nil, adminv1.ErrorInvalidArgument("HTTPS 服务名称格式不正确")
	}
...
func parseLoadBalancingPolicy(
	...
	default:
		return "", adminv1.ErrorInvalidArgument("负载均衡方式不正确")
	}
}

func parseModelForCreate(config *adminv1.ModelServiceInput) (*resource.ModelUpstream, error) {
	if config == nil {
		return nil, nil
	}
	if config.GetClearApiKey() {
		return nil, adminv1.ErrorInvalidArgument("创建模型服务时不能清除 API Key")
	}
	model, _, err := parseModel(config)
	return model, err
}

func parseModelForUpdate(config *adminv1.ModelServiceInput) (*resource.ModelUpstream, bool, error) {
	if config == nil {
...
func parseModel(config *adminv1.ModelServiceInput) (*resource.ModelUpstream, bool, error) {
	if config.ApiKey != nil && config.GetClearApiKey() {
		return nil, false, adminv1.ErrorInvalidArgument("不能同时填写新 API Key 和清除已有 API Key")
	}
	...
	default:
		return nil, false, adminv1.ErrorInvalidArgument("请选择模型服务协议")
	}
	...
	if apiKey == "" || !apivalidation.IsValidModelAPIKey(apiKey) {
		return nil, false, adminv1.ErrorInvalidArgument("API Key 不能为空、包含无效字符或超过长度限制")
	}
	model.APIKey = apiKey
