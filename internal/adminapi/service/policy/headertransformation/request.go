...
func parseSpec(
	...
	requestRules, err := parseRules(requestRuleConfigs, "请求")
	if err != nil {
		return resource.HeaderTransformationPolicySpec{}, err
	}
	responseRules, err := parseRules(responseRuleConfigs, "响应")
	if err != nil {
		return resource.HeaderTransformationPolicySpec{}, err
	}
...
func parseRules(
	...
	for i, config := range configs {
		if config == nil {
			return nil, adminv1.ErrorInvalidArgument("第 %d 条%s Header 规则不能为空", i+1, direction)
		}
		operation, err := parseOperation(config.GetOperation())
		if err != nil {
			return nil, err
		}
		headerName := httpheader.NormalizeName(config.GetName())
		if !httpheader.IsValidName(headerName) {
			return nil, adminv1.ErrorInvalidArgument("第 %d 条%s Header 规则的名称格式不正确", i+1, direction)
		}
		headerValue, err := parseValue(
			operation,
			config.GetValue(),
			i,
			direction,
...
func parseValue(
	...
	case resource.HeaderTransformationRemove:
		if value != "" {
			return "", adminv1.ErrorInvalidArgument("第 %d 条%s Header 删除规则不能配置值", ruleIndex+1, direction)
		}
		return "", nil
	case resource.HeaderTransformationRename:
		value = httpheader.NormalizeName(value)
		if !httpheader.IsValidName(value) {
			return "", adminv1.ErrorInvalidArgument("第 %d 条%s Header 重命名规则的新名称格式不正确", ruleIndex+1, direction)
		}
		return value, nil
	case resource.HeaderTransformationReplace,
		...
		if !httpheader.IsValidValue(value) {
			return "", adminv1.ErrorInvalidArgument("第 %d 条%s Header 规则的值格式不正确", ruleIndex+1, direction)
		}
		return value, nil
	default:
		return "", adminv1.ErrorInvalidArgument("Header 转换操作不正确")
