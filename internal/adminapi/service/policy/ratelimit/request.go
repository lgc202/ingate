...
func parseSpec(
	...
	subject, err := parseSubject(subjectConfig)
	if err != nil {
		return resource.RateLimitPolicySpec{}, err
	}
	limit, err := parseLimit(limitConfig)
	if err != nil {
		return resource.RateLimitPolicySpec{}, err
	}
...
func parseSubject(config *adminv1.RateLimitSubject) (resource.RateLimitSubject, error) {
	if config == nil {
...
func parseLimit(config *adminv1.RateLimit) (resource.RateLimit, error) {
	if config == nil {
