...
func parseSpec(
	...
	targets, err := target.Refs(targetConfigs, resource.KindCaller)
	if err != nil {
		return resource.TokenQuotaPolicySpec{}, err
	}
	timeZone, _, valid := apivalidation.LoadLocation(timeZone)
	if !valid {
		return resource.TokenQuotaPolicySpec{}, adminv1.ErrorInvalidArgument("额度周期时区不正确")
	}
	limits, err := parseLimits(limitConfigs)
	if err != nil {
		return resource.TokenQuotaPolicySpec{}, err
	}
...
func parseLimits(
	...
	if len(configs) > apivalidation.MaxLimits {
	...
	for i, config := range configs {
		...
		period, err := parsePeriod(config.GetPeriod())
		if err != nil {
			return nil, err
		}
		if seen[period] {
...
func periodOrder(period resource.TokenQuotaPeriod) int {
	switch period {
