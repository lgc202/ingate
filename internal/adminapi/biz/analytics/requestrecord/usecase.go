// Package requestrecord 提供控制台请求记录查询用例。
...
var (
	...
	ErrNotFound = adminv1.ErrorRequestRecordNotFound("请求记录不存在或已超过明细保留期")
	// ErrUnavailable 表示请求分析组件当前无法提供查询。
	ErrUnavailable = Unavailable(nil)
)
...
func Unavailable(cause error) error {
	err := adminv1.ErrorDependencyUnavailable("请求记录服务暂时不可用，请稍后重试")
	if cause == nil {
		return err
	}
