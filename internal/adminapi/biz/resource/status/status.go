// Package status 将声明式资源 Condition 转换为产品状态。
package status
...
// ForWasmPlugin 只根据插件制品的下载与校验结果判断安装状态。
// 插件是否作用到流量由引用它的强类型策略状态表达，
// 不与整套 Envoy 配置发布状态耦合。
func ForWasmPlugin(generation int64, conditions []metav1.Condition) Status {
	accepted := currentCondition(generation, conditions, resource.ConditionAccepted)
	if accepted != nil && accepted.Status == metav1.ConditionFalse {
		return FromErrorCondition(accepted)
	}
...
// ForEnabledResource 同时考虑资源开关和当前版本是否已被控制面处理。
func ForEnabledResource(generation int64, enabled bool, conditions []metav1.Condition) Status {
	if !enabled && IsApplied(generation, conditions) {
		return disabled()
	}
	return FromConditions(generation, conditions)
}

// IsApplied 判断当前版本是否已经完成发布或确认没有可应用目标。
func IsApplied(generation int64, conditions []metav1.Condition) bool {
	programmed := ProgrammedCondition(generation, conditions)
	if programmed == nil {
		return false
	}
...
// ProgrammedCondition 返回当前版本的发布状态；状态尚未生成或属于旧版本时返回 nil。
func ProgrammedCondition(generation int64, conditions []metav1.Condition) *metav1.Condition {
	return currentCondition(generation, conditions, resource.ConditionProgrammed)
}

// ResolvedRefsCondition 返回当前版本的引用检查状态，并指示控制面是否曾生成过该状态。
func ResolvedRefsCondition(generation int64, conditions []metav1.Condition) (*metav1.Condition, bool) {
	return conditionForGeneration(generation, conditions, resource.ConditionResolvedRefs)
}

// FromErrorCondition 将失败 Condition 转换为安全的产品状态。
func FromErrorCondition(condition *metav1.Condition) Status {
