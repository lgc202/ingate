...
func (uc *Usecase) List(
	...
	return query.FilterPage(ctx, page, uc.store.ListPage, func(certificate resource.Certificate) bool {
		resourceStatus := status.FromConditions(
			certificate.Generation,
			certificate.Status.Conditions,
		)
		return filter.Match(certificate.Spec.DisplayName, true, resourceStatus.State)
	})
}
