...
func (uc *Usecase) List(
	...
	return query.FilterPage(ctx, page, uc.store.ListPage, func(gateway resource.Gateway) bool {
		...
		return filter.Match(searchText.String(), gateway.Spec.Enabled, resourceStatus.State)
	})
}
