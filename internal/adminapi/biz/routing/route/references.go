...
func (uc *Usecase) checkReferences(ctx context.Context, spec resource.RouteSpec) error {
	...
	for _, gatewayID := range spec.GatewayRefs {
		if gateways[gatewayID] == nil {
			return adminv1.ErrorResourceReferenceNotFound("关联网关 %q 不存在", gatewayID)
		}
	}
	...
	for _, serviceRef := range spec.UpstreamRefs {
		...
		if service == nil {
			return adminv1.ErrorResourceReferenceNotFound("关联服务 %q 不存在", serviceRef.Name)
		}
		if service.Spec.Model != nil {
			return adminv1.ErrorBusinessRuleViolation("模型服务 %q 只能用于 AI 路由", service.Spec.DisplayName)
		}
	}
	...
	for _, model := range spec.AI.Models {
		for _, target := range model.Targets {
			...
			if service == nil {
				return adminv1.ErrorResourceReferenceNotFound("关联模型服务 %q 不存在", target.UpstreamRef)
			}
			if service.Spec.Model == nil {
				return adminv1.ErrorBusinessRuleViolation("服务 %q 不是模型服务", service.Spec.DisplayName)
			}
		}
	}
