...
func (uc *Usecase) List(
	...
	return query.FilterPage(ctx, page, uc.store.ListPage, func(plugin resource.WasmPlugin) bool {
		...
		return filter.Match(searchText, true, resourceStatus.State)
	})
}
...
func (uc *Usecase) Install(
	...
	if !available {
		return nil, adminv1.ErrorBusinessRuleViolation("插件包 %q 不在选定插件源中", packageName)
	}
	plugin, err := uc.store.Create(ctx, apivalidation.PluginID(spec.Package), spec)
	if adminv1.IsResourceAlreadyExists(err) {
		return nil, adminv1.ErrorResourceAlreadyExists("插件包 %q 已安装；如需切换来源，请先卸载现有插件", spec.Package).WithCause(err)
	}
	return plugin, err
}
...
func (uc *Usecase) Upgrade(
	...
	if !available {
		return nil, adminv1.ErrorBusinessRuleViolation("插件包 %q 不在当前插件目录中，无法自动升级", current.Spec.Package)
	}
...
func (uc *Usecase) checkNotUsed(ctx context.Context, plugin *resource.WasmPlugin) error {
	...
	usage := usages[0]
	return adminv1.ErrorResourceReferenced(
