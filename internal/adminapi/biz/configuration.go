...
// ConfigurationUsecase 承载配置状态聚合用例
...
// NewConfigurationUsecase 创建配置发布状态查询用例
func NewConfigurationUsecase(
	gateways GatewayRepository,
	routes RouteRepository,
	upstreams UpstreamRepository,
	certificates CertificateRepository,
...
func (s *ConfigurationUsecase) Get(ctx context.Context) (*ConfigurationReport, error) {
	...
	items := make([]ConfigurationItem, 0, total)
	...
	for _, upstream := range resources.upstreams {
		items = append(items, ConfigurationItem{
			...
			Status: ResourceStatusFromConditions(upstream.Generation, upstream.Status.Conditions),
		})
	}
	for _, certificate := range resources.certificates {
		items = append(items, ConfigurationItem{
			...
			Status: ResourceStatusFromConditions(certificate.Generation, certificate.Status.Conditions),
		})
	}
	...
	return &ConfigurationReport{Summary: summarize(items), Items: items}, nil
}

func (s *ConfigurationUsecase) listResources(ctx context.Context) (resourceLists, error) {
...
func compareItems(a, b ConfigurationItem) int {
