...
func aiUsageTrend(points []*analyticsv1.AIUsageTrendPoint) ([]aiusagebiz.TrendPoint, error) {
	trend := make([]aiusagebiz.TrendPoint, 0, len(points))
	for _, point := range points {
		...
		trend = append(trend, aiusagebiz.TrendPoint{
...
func aiUsageBreakdown(items []*analyticsv1.AIUsageBreakdownItem) ([]aiusagebiz.BreakdownItem, error) {
	breakdown := make([]aiusagebiz.BreakdownItem, 0, len(items))
	for _, item := range items {
		...
		breakdown = append(breakdown, aiusagebiz.BreakdownItem{
