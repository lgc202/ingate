...
func TestCompilerBuildsClusterForEndpointAddress(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if result.HasErrors() {
				t.Fatalf("Compile(endpoint address %q) diagnostics = %v, want no errors", tt.address, result.Diagnostics)
			}
			if len(result.Config.Clusters) != 1 {
				t.Fatalf("Compile(endpoint address %q) cluster count = %d, want 1", tt.address, len(result.Config.Clusters))
			}

			cluster := result.Config.Clusters[0]
			if got, want := cluster.Name, "backend"; got != want {
				t.Errorf("Compile(endpoint address %q) cluster name = %q, want %q", tt.address, got, want)
			}
			if got := cluster.GetType(); got != tt.wantType {
				t.Errorf("Compile(endpoint address %q) cluster type = %v, want %v", tt.address, got, tt.wantType)
			}
			if got := len(result.Config.Endpoints); got != tt.wantEndpointCount {
				t.Errorf("Compile(endpoint address %q) EDS resource count = %d, want %d", tt.address, got, tt.wantEndpointCount)
			}
			if got := cluster.GetLoadAssignment() != nil; got != tt.wantInline {
				t.Errorf("Compile(endpoint address %q) inline load assignment = %t, want %t", tt.address, got, tt.wantInline)
			}
		})
	}
}
...
func TestCompilerBuildsTLSClusterForOpenAIUpstream(t *testing.T) {
	...
	if result.HasErrors() {
		t.Fatalf("Compile(OpenAI TLS upstream) diagnostics = %v, want no errors", result.Diagnostics)
	}
	if len(result.Config.Clusters) != 1 {
		t.Fatalf("Compile(OpenAI TLS upstream) cluster count = %d, want 1", len(result.Config.Clusters))
	}

	transportSocket := result.Config.Clusters[0].TransportSocket
	if transportSocket == nil {
		t.Fatal("Compile(OpenAI TLS upstream) transport socket = nil, want TLS transport socket")
	}
	if got, want := transportSocket.Name, tlsTransportSocketName; got != want {
		t.Errorf("Compile(OpenAI TLS upstream) transport socket name = %q, want %q", got, want)
	}
...
func TestCompilerOmitsSNIForIPTLSIdentity(t *testing.T) {
	...
	if result.HasErrors() {
		t.Fatalf("Compile(IP TLS identity) diagnostics = %v, want no errors", result.Diagnostics)
	}
...
func TestCompilerRejectsUpstreamWithoutType(t *testing.T) {
	...
	if !result.HasErrors() {
		t.Fatal("Compile(upstream without type) has errors = false, want true")
	}
	if !containsDiagnostic(result.Diagnostics, gatewayv1.KindUpstream, upstream.Name, ReasonInvalidSpec) {
		t.Errorf(
			"Compile(upstream without type) diagnostics = %v, want Upstream %q reason %q",
			result.Diagnostics,
			upstream.Name,
			ReasonInvalidSpec,
		)
