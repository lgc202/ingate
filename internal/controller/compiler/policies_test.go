...
func TestPolicyConfigDeduplicatesPolicyAndUsesMostSpecificScope(t *testing.T) {
	...
	compiledTargets := context.compiledPolicyTargets()
	wantTargets := []CompiledPolicyTarget{
		{
	...
	if !slices.Equal(compiledTargets, wantTargets) {
		t.Fatalf("compilation.compiledPolicyTargets() = %v, want %v", compiledTargets, wantTargets)
	}
}
...
func TestAccessControlPolicyRejectsUntrustedIdentityConditions(t *testing.T) {
	for _, conditionType := range []gatewayv1.AccessControlConditionType{"Consumer", "Tenant"} {
		t.Run(string(conditionType), func(t *testing.T) {
			context := &compilation{diagnosticSet: make(map[string]bool)}
			...
			if _, valid := context.accessControlPolicy(policy); valid {
				t.Errorf("compilation.accessControlPolicy(type=%q) valid = true, want false", conditionType)
			}
		})
	}
}
	...
	for _, keyType := range []gatewayv1.RateLimitKeyType{"Consumer", "Tenant", "JWTClaim", "APIKey"} {
		t.Run(string(keyType), func(t *testing.T) {
			...
			if _, valid := context.rateLimitPolicy(policy); valid {
				t.Errorf("compilation.rateLimitPolicy(type=%q) valid = true, want false", keyType)
			}
		})
	}
}
...
func TestPolicyConfigRejectsNamesOnUnnamedDimensions(t *testing.T) {
	...
	if _, valid := context.accessControlPolicy(accessPolicy); valid {
		t.Error("compilation.accessControlPolicy(IP name) valid = true, want false")
	}
	...
	if _, valid := context.rateLimitPolicy(ratePolicy); valid {
		t.Error("compilation.rateLimitPolicy(IP name) valid = true, want false")
	}
}
...
func TestPolicyConfigRejectsNonErrorResponseStatus(t *testing.T) {
	...
	if _, valid := context.accessControlPolicy(accessPolicy); valid {
		t.Error("compilation.accessControlPolicy(status=302) valid = true, want false")
	}
	...
	if _, valid := context.rateLimitPolicy(ratePolicy); valid {
		t.Error("compilation.rateLimitPolicy(status=302) valid = true, want false")
	}
}

func newPolicyTestContext(listener listenerKey, policy *gatewayv1.RateLimitPolicy) *compilation {
	return &compilation{
