...
// Request 表示第一阶段支持的 OpenAI-compatible Chat Completions 请求
//
...
// DecodeRequest 一次性解析并校验第一阶段支持的 OpenAI-compatible 请求
func DecodeRequest(body []byte) (Request, error) {
	if !isJSONObject(body) {
		return Request{}, fmt.Errorf("%w: request body must be a JSON object", llm.ErrInvalidRequest)
	}

	var fields map[string]json.RawMessage
	if err := json.Unmarshal(body, &fields); err != nil {
		return Request{}, fmt.Errorf("%w: decode request body: %v", llm.ErrInvalidRequest, err)
	}
	...
	for _, name := range []string{"tools", "tool_choice", "functions", "function_call", "parallel_tool_calls"} {
		if value, ok := fields[name]; ok && !bytes.Equal(bytes.TrimSpace(value), []byte("null")) {
			return Request{}, fmt.Errorf("%w: field %q is not supported", llm.ErrUnsupportedFeature, name)
		}
	}
	...
	if strings.TrimSpace(request.Model) == "" {
		return Request{}, fmt.Errorf("%w: field %q must not be empty", llm.ErrInvalidRequest, "model")
	}
	...
	if request.Temperature != nil && (*request.Temperature < 0 || *request.Temperature > 2) {
		return Request{}, fmt.Errorf("%w: field %q must be between 0 and 2", llm.ErrInvalidRequest, "temperature")
	}
	if err := decodeOptional(fields, "top_p", &request.TopP); err != nil {
		return Request{}, err
	}
	if request.TopP != nil && (*request.TopP < 0 || *request.TopP > 1) {
		return Request{}, fmt.Errorf("%w: field %q must be between 0 and 1", llm.ErrInvalidRequest, "top_p")
	}
	if err := decodeOptional(fields, "max_tokens", &request.MaxTokens); err != nil {
		return Request{}, err
	}
	if request.MaxTokens != nil && *request.MaxTokens <= 0 {
		return Request{}, fmt.Errorf("%w: field %q must be greater than 0", llm.ErrInvalidRequest, "max_tokens")
	}
	...
	if err := request.validateSupported(); err != nil {
		return Request{}, err
	}
	return request, nil
}

func (r Request) validateSupported() error {
	if len(r.unknownFields) > 0 {
		return fmt.Errorf("%w: fields %s are not supported",
			llm.ErrUnsupportedFeature, strings.Join(r.unknownFields, ", "))
	}
...
func decodeMessages(raw json.RawMessage) ([]Message, error) {
	if len(raw) == 0 {
		return nil, fmt.Errorf("%w: field %q is required", llm.ErrInvalidRequest, "messages")
	}

	var values []map[string]json.RawMessage
	if err := json.Unmarshal(raw, &values); err != nil {
		return nil, fmt.Errorf("%w: field %q must be an array of message objects", llm.ErrInvalidRequest, "messages")
	}
	if len(values) == 0 {
		return nil, fmt.Errorf("%w: field %q must not be empty", llm.ErrInvalidRequest, "messages")
	}

	messages := make([]Message, 0, len(values))
	for i, fields := range values {
		if fields == nil {
			return nil, fmt.Errorf("%w: messages[%d] must be an object", llm.ErrInvalidRequest, i)
		}

		for _, name := range []string{"tool_calls", "function_call"} {
			if value, ok := fields[name]; ok && !bytes.Equal(bytes.TrimSpace(value), []byte("null")) {
				return nil, fmt.Errorf("%w: messages[%d].%s is not supported", llm.ErrUnsupportedFeature, i, name)
			}
		}

		var role Role
		if err := decodeRequiredString(fields, "role", &role); err != nil {
			return nil, fmt.Errorf("%w: messages[%d].role must be system, user, or assistant", llm.ErrInvalidRequest, i)
		}
		if !validRole(role) {
			return nil, fmt.Errorf("%w: messages[%d].role %q is not supported", llm.ErrUnsupportedFeature, i, role)
		}

		rawContent, ok := fields["content"]
		if !ok {
			return nil, fmt.Errorf("%w: messages[%d].content is required", llm.ErrInvalidRequest, i)
		}
		trimmedContent := bytes.TrimSpace(rawContent)
		if len(trimmedContent) > 0 && (trimmedContent[0] == '[' || trimmedContent[0] == '{') {
			return nil, fmt.Errorf("%w: messages[%d].content must be plain text", llm.ErrUnsupportedFeature, i)
		}
		var content string
		if err := decodeRequiredString(fields, "content", &content); err != nil {
			return nil, fmt.Errorf("%w: messages[%d].content must be a string", llm.ErrInvalidRequest, i)
		}
...
func decodeRequiredString[T ~string](fields map[string]json.RawMessage, name string, target *T) error {
	raw, ok := fields[name]
	if !ok {
		return fmt.Errorf("%w: field %q is required", llm.ErrInvalidRequest, name)
	}
	if bytes.Equal(bytes.TrimSpace(raw), []byte("null")) {
		return fmt.Errorf("%w: field %q must be a string", llm.ErrInvalidRequest, name)
	}
...
func decodeOptional[T any](fields map[string]json.RawMessage, name string, target **T) error {
	...
	if bytes.Equal(bytes.TrimSpace(raw), []byte("null")) {
		return fmt.Errorf("%w: field %q has an invalid type", llm.ErrInvalidRequest, name)
	}
	var value T
...
func decodeStop(raw json.RawMessage) ([]string, error) {
	...
	if err := json.Unmarshal(raw, &multiple); err != nil || multiple == nil {
		return nil, fmt.Errorf("%w: field %q must be a string or an array of strings", llm.ErrInvalidRequest, "stop")
	}
	return multiple, nil
}
