...
acl-plugin-test:
	cd $(ACL_PLUGIN_DIR) && GOCACHE=$(GO_CACHE_DIR) go test ./...

acl-plugin-build:
	mkdir -p _output/plugins
	cd $(ACL_PLUGIN_DIR) && GOOS=wasip1 GOARCH=wasm GOCACHE=$(GO_CACHE_DIR) go build -buildmode=c-shared -o ../../$(ACL_PLUGIN_OUT) .
