...
所有 Go 服务的应用日志由 Kratos Log 输出到标准错误流，容器、systemd 或日志采集器负责收集、落盘和轮转；etcd、MySQL、Redis、Kafka 和 ClickHouse 数据分别保存在独立 Volume。Assistant 的 YAML 只包含 MySQL、Redis、Admin API 和进程运行参数；模型连接由管理员在 Console 中配置并持久化到 MySQL，模型凭据不会写入 YAML 或通过查询接口回显。
