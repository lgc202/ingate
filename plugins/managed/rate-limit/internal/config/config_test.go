...
func TestParseRouteConfigDefaultsSchemaVersion(t *testing.T) {
	...
	if cfg.SchemaVersion != SchemaVersionV1 {
		t.Fatalf("SchemaVersion = %q, want %q", cfg.SchemaVersion, SchemaVersionV1)
	}
}

func TestParseRouteConfigAcceptsTypedStructEnvelope(t *testing.T) {
	cfg, err := ParseRouteConfig([]byte(`{
		"typeUrl":"type.googleapis.com/ingate.extensions.filters.http.managed_rate_limit.v1.RouteConfig",
